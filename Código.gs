    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][2] === email) {
        return {
          ...
          estado: data[i][4],
          fechaRegistro:
            data[i][5] instanceof Date ? data[i][5].toISOString() : "",
        };
      }
    }
    return null;
...
function crearActividad(datos) {
  try {
    ...
    Logger.log(
      "ColaboradoresAdicionales: " + colaboradoresAdicionales.join(","),
    );
    ...
    if (usuario.rol === "Jefatura" || usuario.rol === "Colaborador") {
      ...
      if (ini && fin) {
        const cruce = verificarCruceYSugerencias(
          datos.responsable,
          ini,
          fin,
          null,
    ...
    try {
      registrarHistorial(id, usuario.email, "Creación", "Actividad creada");
    } catch (e) {
      Logger.log("Error historial: " + e);
    }
    try {
      enviarNotificacionNuevaActividad({ ...datos, id });
    } catch (e) {
      Logger.log("Error notif: " + e);
    }
    try {
      crearEventoCalendario({ ...datos, id, estado: CONFIG.ESTADOS.PENDIENTE });
    } catch (e) {
...
function obtenerActividades(filtros = {}) {
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const jefaturasAdicionales = data[i][19]
        ? String(data[i][19])
            .split(",")
            .map((e) => e.trim())
            .filter(Boolean)
        : [];
      const colaboradoresAdicionales = data[i][20]
        ? String(data[i][20])
            .split(",")
            .map((e) => e.trim())
            .filter(Boolean)
      ...
      const actividad = {
        ...
        fechaInicio:
          data[i][9] instanceof Date
            ? data[i][9].toISOString()
            : data[i][9] || "",
        fechaFin:
          data[i][10] instanceof Date
            ? data[i][10].toISOString()
            : data[i][10] || "",
        fechaCreacion:
        ...
        ultimaActualizacion:
          data[i][12] instanceof Date
            ? data[i][12].toISOString()
            : data[i][12] || "",
        avance: data[i][13] || 0,
        tipo: data[i][14] || "Recurrente",
        fechaAceptacion: data[i][15]
          ? new Date(data[i][15]).toISOString()
          : null,
        tiempoEjecucion: data[i][16] || 0,
        estadoAnterior: data[i][17] || "",
        fechaSuspension: data[i][18]
          ? new Date(data[i][18]).toISOString()
          : null,
        jefaturasAdicionales,
        colaboradoresAdicionales,
      };
...
function marcarActividadesVencidas() {
  try {
    ...
    const sheet = ss.getSheetByName("Actividades");
    if (!sheet) {
      Logger.log("Sin hoja Actividades");
      return;
    }
...
function actualizarActividad(id, cambios) {
  try {
    ...
    const fila = sheet
      .getRange(filaIdx, 1, 1, sheet.getLastColumn())
      .getValues()[0];
    ...
    if (cambios.estado === "Completada") {
      const fechaFinEfectiva = cambios.fechaFin
        ? new Date(cambios.fechaFin)
        : fechaFin;
    ...
    const estadoFinal =
      cambios.estado !== undefined ? cambios.estado : estadoActual;
    if (
      estadoFinal === "Completada" ||
      estadoFinal === "Completada con Atraso"
    ) {
      cambios.avance = 100;
    ...
    if (cambios.avance !== undefined && cambios.estado === undefined) {
      ...
      if (avance === 100) {
        ...
        cambios.estado =
          estadoActual === "Vencida" || yaVencioFecha
    ...
    try {
      registrarHistorial(
        ...
        "Estado: " +
          ...
          (cambios.avance !== undefined
            ? " | Avance: " + cambios.avance + "%"
            : ""),
      );
    } catch (e) {
      Logger.log("Error historial: " + e);
    ...
    if (
      ...
      try {
        ...
        Logger.log("Error email completado: " + e);
      }
    }
...
— Sistema de Actividades | ZaazMago
    ...
    if (actividad.responsable) {
      try {
        MailApp.sendEmail({
          to: actividad.responsable,
          subject: asunto,
          body: cuerpo,
        });
        Logger.log("Email completado enviado a: " + actividad.responsable);
      } catch (e) {
        Logger.log("Error email responsable: " + e);
      }
    }
...
function validarPermisosEdicion(usuario, actividad) {
  switch (usuario.rol) {
    ...
    case CONFIG.ROLES.JEFATURA:
      return (
        usuario.email === actividad.jefe ||
        usuario.email === actividad.responsable
      );
    case CONFIG.ROLES.COLABORADOR:
...
function agregarComentario(actividadId, comentario) {
  ...
  sheet.appendRow([id, actividadId, usuario.email, comentario, new Date()]);
...
function crearEventoCalendario(actividad) {
  try {
    ...
    if (isNaN(fechaIni) || isNaN(fechaFin)) {
      Logger.log(
        "Fechas inválidas para calendario: " +
...
function exportarActividadAlCalendario(actividadId) {
  try {
    ...
    if (colorMap[actividad.prioridad])
      evento.setColor(colorMap[actividad.prioridad]);
    evento.addPopupReminder(30);
    evento.addEmailReminder(60);
  ...
  try {
    ...
    actividades.forEach(function (act) {
      try {
        ...
        const evento = calendario.createEvent(act.titulo, inicio, fin, {
          description: [
            "Estado: " + act.estado,
    ...
    Logger.log(
      "✅ Exportación completa. Creados: " + creados + " | Errores: " + errores,
    );
    return {
      success: true,
      message:
...
function enviarNotificacionNuevaActividad(actividad) {
  try {
    ...
    const fechaIni = actividad.fechaInicio
...
function registrarHistorial(actividadId, usuario, accion, detalle) {
  ...
  sheet.appendRow([id, actividadId, usuario, accion, detalle, new Date()]);
}
...
function generarReporte(filtros) {
  ...
  actividades.forEach((act) => {
    // Por estado
    estadisticas.porEstado[act.estado] =
      (estadisticas.porEstado[act.estado] || 0) + 1;

    // Por prioridad
    estadisticas.porPrioridad[act.prioridad] =
      (estadisticas.porPrioridad[act.prioridad] || 0) + 1;

    // Por responsable
    estadisticas.porResponsable[act.responsable] =
      (estadisticas.porResponsable[act.responsable] || 0) + 1;
  });
  ...
  try {
    ...
    const usuarioSerializable = {
      ...
      fechaRegistro: usuario.fechaRegistro
        ? new Date(usuario.fechaRegistro).toISOString()
        : null,
    };
...
function crearUsuariosPrueba() {
  ...
  return {
    success: true,
    message: "Se crearon " + usuariosPrueba.length + " usuarios de prueba",
  };
}
...
function crearProyecto(datos) {
  try {
    ...
    if (!sheet) {
      throw new Error(
        "La hoja Proyectos no existe. Ejecuta inicializarHojaProyectos()",
      );
    }
    ...
    const jefeResponsable =
      datos.jefeResponsable ||
      (usuario.rol === "Jefatura" ? usuario.email : "");
    const gerenteResponsable =
      datos.gerenteResponsable ||
      (usuario.rol === "Gerencia" ? usuario.email : "");
    ...
    registrarHistorial(
      ...
      usuario.email,
      "Creación Proyecto",
      "Proyecto creado: " + proyecto.nombre,
    );
    ...
    const proyectoSerializable = {
      ...
      fechaInicio:
        proyecto.fechaInicio instanceof Date
          ? proyecto.fechaInicio.toISOString()
          : proyecto.fechaInicio || "",
      fechaFin:
        proyecto.fechaFin instanceof Date
          ? proyecto.fechaFin.toISOString()
          : proyecto.fechaFin || "",
      fechaCreacion:
        proyecto.fechaCreacion instanceof Date
          ? proyecto.fechaCreacion.toISOString()
          : proyecto.fechaCreacion || "",
      ultimaActualizacion:
        proyecto.ultimaActualizacion instanceof Date
          ? proyecto.ultimaActualizacion.toISOString()
          : proyecto.ultimaActualizacion || "",
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      switch (usuario.rol) {
        ...
        case CONFIG.ROLES.JEFATURA:
          incluir =
            proyecto.jefeResponsable === usuario.email ||
      ...
      if (
        incluir &&
        filtros.prioridad &&
        proyecto.prioridad !== filtros.prioridad
      )
        incluir = false;
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][0] === id) {
        ...
        if (cambios.nombre !== undefined)
          sheet.getRange(fila, 2).setValue(cambios.nombre);
        if (cambios.descripcion !== undefined)
          sheet.getRange(fila, 3).setValue(cambios.descripcion);
        if (cambios.gerenteResponsable !== undefined)
          sheet.getRange(fila, 4).setValue(cambios.gerenteResponsable);
        if (cambios.jefeResponsable !== undefined)
          sheet.getRange(fila, 5).setValue(cambios.jefeResponsable);
        ...
        if (cambios.estado !== undefined)
          sheet.getRange(fila, 7).setValue(cambios.estado);
        if (cambios.prioridad !== undefined)
          sheet.getRange(fila, 8).setValue(cambios.prioridad);
        if (cambios.fechaInicio !== undefined)
          sheet.getRange(fila, 9).setValue(new Date(cambios.fechaInicio));
        if (cambios.fechaFin !== undefined)
          sheet.getRange(fila, 10).setValue(new Date(cambios.fechaFin));
        if (cambios.presupuestoEstimado !== undefined)
          sheet.getRange(fila, 13).setValue(cambios.presupuestoEstimado);
        if (cambios.avanceGeneral !== undefined)
          sheet.getRange(fila, 14).setValue(cambios.avanceGeneral);

        // Actualizar timestamp
        sheet.getRange(fila, 12).setValue(new Date());

        // Registrar en historial
        const detalles = Object.keys(cambios)
          .map((key) => `${key}: ${cambios[key]}`)
          .join(", ");
        registrarHistorial(
          id,
          usuario.email,
...
function obtenerActividadesDeProyecto(proyectoId) {
  try {
    ...
    Logger.log(
      "Actividades del proyecto " + proyectoId + ": " + actividades.length,
    );
    return actividades;
  } catch (error) {
    Logger.log(
      "❌ Error al obtener actividades del proyecto: " + error.toString(),
    );
    return [];
  }
}
...
function calcularCargaTrabajo(emailColaborador) {
  try {
    ...
    for (let i = 1; i < dataProyectos.length; i++) {
      const colaboradores = dataProyectos[i][5]
        ? dataProyectos[i][5].split(",")
        : [];
      const estado = dataProyectos[i][6];

      if (
        colaboradores.includes(emailColaborador) &&
        estado !== "Completado" &&
        estado !== "Cancelado"
      ) {
        proyectosActivos++;
    ...
    const cargaTrabajo = {
      ...
      nivelCarga: calcularNivelCarga(
        proyectosActivos,
        actividadesPendientes + actividadesEnProceso,
      ),
    };
...
function obtenerCargaTrabajoEquipo() {
  try {
    ...
    cargasTrabajo.sort(
      (a, b) => ordenCarga[b.nivelCarga] - ordenCarga[a.nivelCarga],
    );
...
function actualizarAvanceProyecto(proyectoId) {
  try {
    ...
    Logger.log(
      "✅ Avance del proyecto " + proyectoId + " actualizado: " + avance + "%",
    );

    return { success: true, avance: avance };
  } catch (error) {
    Logger.log(
      "❌ Error al actualizar avance del proyecto: " + error.toString(),
    );
    throw error;
  }
}
...
function enviarNotificacionNuevoProyecto(proyecto) {
  try {
    ...
    if (proyecto.gerenteResponsable)
      destinatarios.push(proyecto.gerenteResponsable);
...
function calcularAvanceProyecto(proyectoId) {
  try {
    ...
    Logger.log(
      "Total actividades encontradas: " + actividadesDelProyecto.length,
    );
    ...
    // Calcular promedio
    const sumaAvances = actividadesDelProyecto.reduce(
      (suma, act) => suma + act.avance,
      0,
    );
    const avancePromedio = Math.round(
      sumaAvances / actividadesDelProyecto.length,
    );
...
function calcularAvanceProyecto(proyectoId) {
  try {
    ...
    Logger.log(
      "Actividades del proyecto encontradas: " + actividadesDelProyecto.length,
    );
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (String(proyectoIdEnHoja) === String(proyectoId)) {
        ...
        Logger.log(
          "✅ Proyecto encontrado: " + nombreProyecto + " (fila " + fila + ")",
        );
        ...
        Logger.log(
          "✅ PROYECTO ACTUALIZADO: " + nombreProyecto + " → " + avance + "%",
        );
...
function debugEstructuraDatos() {
  ...
  if (dataProy.length > 1) {
    ...
    Logger.log(
      'AvanceGeneral (columna N): "' +
        dataProy[1][13] +
        '" (tipo: ' +
        typeof dataProy[1][13] +
        ")",
    );
  }
...
function obtenerColaboradoresConCarga() {
  try {
    ...
    colaboradoresConCarga.sort(
      (a, b) => a.carga.puntajeCarga - b.carga.puntajeCarga,
    );
  ...
  try {
    ...
    if (!sheet) {
      ...
      sheet.appendRow([
        ...
        "Tipo",
        "FechaAceptacion",
        "TiempoEjecucion", // ← NUEVAS COLUMNAS
      ]);
    }
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][0] === actividadId) {
        ...
        return {
          success: true,
          message:
            "Actividad aceptada. Cronómetro iniciado." +
            (resultadoPausa.pausadas > 0
              ? " " + resultadoPausa.pausadas + " actividad(es) pausada(s)."
              : ""),
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][0] === actividadId) {
        ...
        Logger.log(
          "✅ Actividades reanudadas: " + resultadoReanudar.reanudadas,
        );

        return {
          success: true,
          message:
            "Actividad completada en " +
            tiempoFormateado +
            ". " +
            resultadoReanudar.reanudadas +
            " actividad(es) reanudada(s).",
...
function obtenerActividadesActivasColaborador(emailColaborador) {
  try {
    ...
    Logger.log(
      "Actividades en proceso encontradas: " + actividadesActivas.length,
    );
...
function pausarActividades(actividadIds) {
  try {
    ...
    if (!actividadIds || actividadIds.length === 0) {
      ...
      return {
        success: true,
        message: "Sin actividades a pausar",
        pausadas: 0,
      };
    }
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (responsable === emailColaborador && estado === "Suspendida") {
        ...
        Logger.log(
          "  ✅ Reanudada: " + tituloActividad + " → " + estadoARestaurar,
        );
        reanudadas++;
      }
    }
...
function verificarCruceYSugerencias(
  responsableEmail,
  nuevaInicio,
  nuevaFin,
  excluirId,
  ...
  try {
    ...
    while (diasConSlots < 3 && intentos < 30) {
      ...
      const bloquesDelDia = bloques
        .filter(function (b) {
          return (
            b.ini.toDateString() === diaActual.toDateString() ||
            b.fin.toDateString() === diaActual.toDateString() ||
            (b.ini <= diaActual &&
              b.fin >= new Date(diaActual.getTime() + 86400000))
          );
        })
        .map(function (b) {
          return { ini: b.ini, fin: b.fin };
        })
        .sort(function (a, b) {
          return a.ini - b.ini;
        });
      ...
      if (slotsDelDia.length > 0) {
        ...
        slotsDelDia.slice(0, 2).forEach(function (s) {
          horasLibres.push(s);
        });
        diasConSlots++;
      }
